      ...
      <div className="flex flex-col gap-2 items-center">
        <span className="text-sm text-gray-500 font-medium flex gap-2 items-center">
          <span className="text-xs px-1.5 py-0.5 bg-primary-green rounded-sm text-white flex items-center gap-0.5">
            {ratings?.toFixed(1) || 0} <StarIcon sx={{ fontSize: '14px' }} />
          </span>
          <span>({numOfReviews?.toLocaleString() || 0})</span>
        </span>
        <div className="flex items-center gap-1.5 text-md font-medium">
          <span>₹{price?.toLocaleString() || 0}</span>
          <span className="text-gray-500 line-through text-xs">₹{cuttedPrice?.toLocaleString() || 0}</span>
